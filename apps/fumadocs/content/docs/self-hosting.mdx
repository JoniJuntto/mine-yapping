...
wrapper, which downloads its own JDK 25.
...
No JDK install needed — the Gradle wrapper downloads JDK 25 (Adoptium) on first run, on
...
<Callout title="Minecraft 26.x is deobfuscated">
  There is no `mappings` line in `build.gradle`, and dependencies use plain
  `implementation` rather than `modImplementation`. Yarn and Mojang mapping files do not
  exist for these versions — do not try to add them.
</Callout>
